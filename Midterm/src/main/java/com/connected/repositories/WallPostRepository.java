...
public interface WallPostRepository extends JpaRepository<WallPost, Long> {
	 ...
	 * where the author is the current user or the author is being followed by the
	 * current user
	 */
	...
	@Query("SELECT DISTINCT w FROM WallPost w LEFT JOIN FETCH w.comments" + " WHERE w IN (:wallPosts)")
